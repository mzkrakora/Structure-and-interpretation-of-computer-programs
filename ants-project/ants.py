...
###I dedicate this project to Jeanine, may she be in good health
...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        #entrance becomes an exit for another place, placeb=exit of placea, then placea is
        #entrance to place
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        # change this so the ant throws at a bee in front of it
        # to get the place in front of another place object, use the place's exit attribute
        # the hive is the entrance of the first place in a tunnel
        
        # distance b/w ant & bee
...
class FireAnt(Ant):
    ...
    armor = 1
    food_cost = 5
    # access place instance attribute to obtain the place of the fire ant
    # to access the bees in a location, call the bee instance attribute
...
class WallAnt(Ant):
    ...
    def can_contain(self, other):
        return False   # Change to True to view in the GUI
...
class QueenAnt(ScubaThrower):  
    ...
    imposter = False
    implemented = True
    doubling = []  # Change to True to view in the GUI
    ...
    def action(self, colony):
        ...
        backward = self.place
        while backward:
            buff_ant = backward.ant
            if backward.ant and (backward.ant not in QueenAnt.doubling): # unbuffed ant & buffed ant, ie if in doubling then won't enter the suite
                if buff_ant.name != 'Queen': #case for if the ant is unbuffed
                    buff_ant.damage *= 2 #doubles damage
                    QueenAnt.doubling.append(buff_ant) #sets double attribute to true, the ant is now buffed
                    if buff_ant.is_container and buff_ant.name != 'Queen': #ant is container,
                        buff_ant = buff_ant.contained_ant # checks if there is an ant in the container, ie. defines protected ant
                        if buff_ant and (buff_ant not in QueenAnt.doubling): #checks if protected ant exists and unbuffed
                            buff_ant.damage *= 2 #buffs protected ant
                            QueenAnt.doubling.append(buff_ant)
            if buff_ant:
                if buff_ant.is_container and buff_ant in QueenAnt.doubling:
                    buff_ant = buff_ant.contained_ant # checks if there is an ant in the container, ie. defines protected ant
                    if buff_ant and (buff_ant not in QueenAnt.doubling): #checks if protected ant exists and unbuffed
                        buff_ant.damage *= 2 #buffs protected ant
                        QueenAnt.doubling.append(buff_ant)
                    #adds protected buffed ant to list
# thrower ant is protected, not being damage doubled.
#now only checking if container, not doubling container ant
            backward = backward.exit

        # backward = self.place
        # while hasattr(backward, "exit"):
        #     if type(backward.ant) != QueenAnt:
        #         if backward.ant != None and backward.ant == False: #case for if the ant is unbuffed
        #             backward.ant.damage *= 2 #doubles damage
        #             backward.ant.double = True #sets double attribute to true, the ant is now buffed
        #             if backward.ant.is_container and backward.ant.contained_ant != None: #and backward.ant.contained_ant==False: #checks that the ant is a container and if the ant exists
        #                 if not type(backward.ant.contained_ant) == QueenAnt:
        #                     backward.ant.contained_ant.damage *= 2 #buffs container ant
        #                     backward.ant.contained_ant.double = True
        #     backward = backward.exit
